...
def create_height_grid(lipid_grid, M, L, z_mean):
    """ 
    Calculate the height field from the lipid_grid, z_mean is the mean
    height if the lipid heads. 
    //TODO: if we decide to keep it consider refactoring to all grid avraging
    functions """
...
def interpulate_grid(grid, empty_grid_points, M):
    """ bi-linear interpolation for emty grid points. Assumes there are no
...
def interpulate_grid2(grid, empty_grid_points, M):
    """ Interpolates the grid with arbitrary missing points"""
...
def normalize_grid(grid, M):
    """  "Normalize" the vector values at each gridpoint, such that Z=1"""
...
def collect(n_q, w_grid, M, L):
    ...
    # DFT for  the field, and separate the longitudinal
